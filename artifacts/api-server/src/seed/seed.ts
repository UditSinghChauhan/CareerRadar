...
async function main() {
  ...
  // Seeded rows go through the same derivations a synced row gets in
  // providers/normalizer.ts — location (Phase 2.0) and relevance (Phase 2.1)
  // — so a fresh database shows a feed without first running the backfills.
  const derive = (job: InsertJob): InsertJob => {
    const location = toLocationColumns(
      normalizeLocation(job.location, job.country, {
        providerRemote: job.workMode === "remote",
      }),
    );
